...
export default function Tab2Asset() {
  ...
  async function processUpgrade() {
    ...
    let oldAddress = Address.parse(oldA404Address);
    if ('wallet?.account?.address' && oldAddress) {
      if (oldA404Balance == 0) {
        quickToast('Check your old A404 balance', 'Old A404 balance is 0.');
        return;
      }

      console.error(oldAddress);
      let op_update_a404 = 0xe2188ce;
      let forward_payload = beginCell().storeUint(op_update_a404, 32)  //op_code
        .endCell();
  ...
  function sellOnGetgems(isMainnet: boolean, wallet: string | undefined, collection: string) {
    if (window && wallet && collection) {
      ...
      toast({
        title: 'Do you have any A404 NFT to sell? ',
